...
finite-cutoff trace positivity is strictly weaker than Weil positivity and may 
produce false positives; therefore finite truncated trace positivity cannot 
establish the Riemann hypothesis via noncommutative geometry.
\end{abstract}
...
function $\zeta(s)$ lie on the critical line $\Re(s) = \tfrac{1}{2}$. A striking 
empirical observation, due to Montgomery \cite{Montgomery1973} and confirmed 
numerically by Odlyzko \cite{Odlyzko1987}, is that the pair correlation of 
Riemann zeros matches the statistics of eigenvalues of random matrices from 
the Gaussian Unitary Ensemble (GUE). This \emph{Montgomery--Odlyzko law} 
suggests that the zeros should have a spectral interpretation: they should 
arise as eigenvalues of some natural operator.

Among the structural approaches to realizing this spectral picture, two stand 
in close relation:
...
\]
where the error term $E_\Lambda(f)$ is generically nonzero. For 
positive-type inputs $h = f * f^*$, the error satisfies $E_\Lambda(h) \leq 0$ 
for all sufficiently large $\Lambda$: the truncated trace systematically 
undershoots the Weil term.
\end{theorem}
...
\begin{enumerate}
\item[\textup{(i)}] If $\hat{f}(\tfrac{1}{2} + it)$ has unbounded support 
\textup{(}i.e., is nonzero for arbitrarily large $|t|$\textup{)}, then 
$E_\Lambda(f) \neq 0$ for all sufficiently large $\Lambda$.
...
\textbf{Proof of (i):} Assume $\hat{f}(\tfrac{1}{2} + it)$ has unbounded support.
By the Stirling expansion (Lemma \ref{lem:digamma} below),
$\psi_\infty(t) = \tfrac{1}{2}\log(|t|/2) + O(|t|^{-1}) > 0$ for $|t| > 2$.
Since $\hat{f}(\tfrac{1}{2} + it)$ is nonzero for arbitrarily large $|t|$,
there exists $\Lambda_0$ and an interval $I \subset \{|t| > \Lambda_0\}$ with
$|\hat{f}(\tfrac{1}{2} + it)| \geq c > 0$ on $I$. For $\Lambda > \Lambda_0$
such that $I \cap \{|t| > \Lambda\} \neq \emptyset$:
\[
...
\subsection{One-way implication: truncation overestimates positivity}

\begin{corollary}[False positives]\label{cor:positivity}
For positive-type inputs $h = f * f^*$, the truncated functional overestimates 
the Weil functional:
\[
C_\Lambda(h) \geq C_{\textup{Weil}}(h).
\]
In particular:
\begin{enumerate}
\item[\textup{(a)}] $C_{\textup{Weil}}(h) \geq 0 \Rightarrow C_\Lambda(h) \geq 0$.
\item[\textup{(b)}] The converse fails: $C_\Lambda(h) \geq 0$ does not imply 
$C_{\textup{Weil}}(h) \geq 0$.
\end{enumerate}
Truncated positivity is a weaker condition that may produce false positives.
\end{corollary}

\begin{proof}
By Theorem \ref{thm:mismatch}(ii), $E_\Lambda(h) \leq 0$ for $h = f * f^*$ and 
sufficiently large $\Lambda$. Writing the explicit formula contributions:
\[
C_\Lambda(h) = (\text{spectral sum})_\Lambda = (\text{other terms}) - T_\Lambda(h)
\]
\[
C_{\textup{Weil}}(h) = (\text{other terms}) - W_\infty(h)
\]
Since $T_\Lambda(h) = W_\infty(h) + E_\Lambda(h) \leq W_\infty(h)$, we have 
$-T_\Lambda(h) \geq -W_\infty(h)$, hence $C_\Lambda(h) \geq C_{\textup{Weil}}(h)$.

For (b): since $h = f * f^*$ with generic $f$ satisfies the hypothesis of 
Theorem \ref{thm:mismatch}(i), we have $E_\Lambda(h) \neq 0$ for large $\Lambda$.
Thus the gap $C_\Lambda(h) - C_{\textup{Weil}}(h) = -E_\Lambda(h) > 0$ is 
nonzero, so there exist $h$ with 
$C_{\textup{Weil}}(h) < 0 < C_\Lambda(h)$ for appropriate $\Lambda$.
\end{proof}

\subsection{Obstruction to the spectral approach}

\begin{corollary}\label{cor:obstruction}
Any proof of the Riemann hypothesis via the Connes program that proceeds by:
\begin{enumerate}
\item[\textup{(a)}] establishing trace positivity $C_\Lambda(f * f^*) \geq 0$ 
at finite cutoff, then
\item[\textup{(b)}] concluding Weil positivity $C_{\textup{Weil}}(f * f^*) \geq 0$,
\end{enumerate}
is obstructed. By Corollary \ref{cor:positivity}, (a) is strictly weaker than (b): 
finite-cutoff positivity overestimates the Weil functional, so it cannot establish RH.
\end{corollary}
...
at finite cutoff by a nonzero error term. For positive-type inputs, this error 
is one-sided: the truncated trace undershoots the Weil term.
...
\item As a consequence, finite-cutoff trace positivity overestimates Weil 
positivity. Truncated positivity cannot establish RH because it is too weak 
a condition---it admits false positives.
